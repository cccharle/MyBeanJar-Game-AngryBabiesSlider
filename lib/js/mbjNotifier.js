...
function getimageforplay(img) {
    image = img.src;
    $('<img class="jqPuzzle jqp-r3-c3-h1-SNABCDE userimg" id="mbj_award_img" src="' + image + '" >').insertAfter(".aaa");
...
function mbjNotifyBuyUserimage(result, message) {
...
function mbjGetUserimage() {
    if (mbjUserLoggedIn) {
        get_user_image(u, p, mbjAppID, mbjNotifyGetUserimage);
...
function RemoveFromCart(imgButton) {
    ...
    jQuery( imgButton ).replaceWith('<button class="btn-mbj btn-iap btn-purchase" onclick="javascript:AddToCart(this)"><span><i class="ion-android-add-circle" style="vertical-align: middle;"></i><br/>$0.99</span></button>');
}
...
function centerOnboardModal(){    
    $('.onboard-modal').each(function(){
        ...
        $(this).css('top',(((($(this).parent().innerHeight() - ($(this).outerHeight())) / 3)) + 'px'));
    });
};
...
function MbjDisplayRegistrationView() {
    jQuery( '#mbj-login-register' ).replaceWith(' <div class="mbj_notification_inner" id="mbj-login-details"> <div class="mbj_notification_title"> <img src="img/mbj_logo_50px.png"/> <h2>SIGN IN AND WIN FOR REAL</h2> </div><div class="mbj-login mbj-form-block" id="mbj_details"> <form class="mbj-form" id="mbj_details_form" method="post"> <div class="element-input"> <label class="title" style="color: #a9a9a9;">e-mail</label> <input class="large" id="mbj_form_reg_email" type="email" name="mbj_reg_email" required/> </div><div class="element-input"> <label class="title" style="color: #a9a9a9;">ZIP / postal code</label> <input class="large" id="mbj_form_reg_zip" type="text" name="mbj_reg_zip" required/> </div><div class="element-input"> <label class="title" style="color: #a9a9a9;">password</label> <input class="large" id="mbj_form_p" type="password" name="mbj_password" required/> </div><div class="mbj_login_status"> </div></form> </div><div class="submit action-buttons"> <button class="btn-mbj full-width" id="mbj-next" onclick="MbjDisplayCategoriesView()"> <span class="mbj-button-icon"><i class="ion-chevron-right" style="font-size: 28px; vertical-align: middle; padding-right:0.5em;"></i></span><span class="mbj-button-text">Next</span> </button> </div></div>');
}
...
// function MbjDeselectCategory(category) {
//     jQuery( category ).removeClass('btn-category-selected');
//     jQuery( category ).children('span.mbj-button-icon').children('i').addClass('ion-android-radio-button-off').removeClass('ion-android-radio-button-on');
// }
