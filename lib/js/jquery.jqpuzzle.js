	...
	$.fn.jqPuzzle = function(settings, texts) {
		...
		var defaults = {
			...
			style: {
				...
				backgroundOpacity: 0.05	// opacity of the original image behind the pieces
...
function changeHintIndicator(indActive){
	if(indActive){
		jQuery("#hintloader").css("opacity", 0.9);
		console.log("Hint indicator activated");
	}
	else{
		jQuery("#hintloader").css("opacity", 0);
		console.log("Hint indicator deactivated");
	}
};
...
function updateHint(thisid){
    ...
    hintActive = true;
    hintCount++;
    jQuery("#hintloader").animate({opacity: 0.9}, 800).delay(800, solvePuzzle());
    // solvePuzzle();
    jQuery("#hintloader").animate({opacity: 0}, 800);
    var thisid = returnId();
    interval = setInterval(function() {highlightAnswers(thisid);}, 500);
}
